use log::info;
use crate::events::{AES128EncryptEvent, MemoryReadRecord, PrecompileEvent, AES_128_BLOCK_BYTES, AES_128_BLOCK_U32S};
...
impl Syscall for AES128EncryptSyscall {
    ...
    fn execute(
        ...
        for i in 0..state.len() {
            log::info!("execute: before r1:  state[{}] = {:x}", i, state[i]);
        }
        ...
        for i in 1..11 {
            ...
            // Subs_bytes
            if i == 1 {
                // for j in 0..AES_128_BLOCK_BYTES {
                for j in 0..2 {
                    log::info!("state: {:?}", state[j]);
                    let (record, value) = rt.mr(sbox_ptr + state[j] as u32 * 4);
                    assert_eq!(value, AES_SBOX[state[j] as usize]);
                    log::info!("record: {:?}", record);
                    log::info!("timestamp: {:?}", rt.clk);
                    sbox_read_records.push(record);
                    assert!(value <= u8::MAX as u32);
                    sbox.push(value as u8);
                    state[j] = value as u8;
